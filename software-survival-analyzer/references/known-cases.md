...
> **These entries are baselines from the last calibration, not permanent truth** (v5). In a
> domain where change is annual, an entry can fall out of date relative to the product's
> actual state. When analyzing a piece of software that already has an entry here:
> (1) run the Prior Phase of Research if appropriate, (2) present **baseline vs. updated**
> side by side, and (3) if the change is stable, propose to the user that the entry be
> updated, with the **calibration date** noted. Entries without a date predate v5 and should
> be treated as undated baselines.

**Fields added in v4** (relative to v3):
...
- **Update (research Jun 2026, v5)** — *baseline above unchanged; this is the scenario with fresh evidence*:
  - Changes vs. baseline: **Fc 30→25** and **Kc 25→22** (investment in agent-facing surface: Dev Mode MCP server, Code Connect, Code-to-Canvas, canvas writing, integrations with Cursor/Claude Code/Codex, MCP customers +70% in seats). The MCP is still beta, which is why Fc does not fall to Vercel's level (10).
  - **Updated base ratio**: (50×60) / (47×10) = **6.4** (🟡 Viable)
  - **H decomposed**: multiplayer collaboration ~85 (durable, Slack-like) vs. design taste ~55 (erodable). Effective H ~72, boost +1.5. The durable moat is the collaboration, not the "taste".
